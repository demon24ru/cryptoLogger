...
* Supports Polymarket crypto prediction markets (e.g. BTC / ETH price `ABOVE` & `RANGE` events). The app auto-discovers the active markets for the configured coins, records the raw CLOB order-book stream (snapshot anchors + deltas) and the real market resolution to ClickHouse. See the [Polymarket prediction markets](#polymarket-prediction-markets) section.
...
(
    ...
    `question` String,
    `outcome_name` String,
    `market_type` String,               -- ABOVE / RANGE
...
* **exchanges : markets : info : market_types** : Which event types to record for this coin, as an array. Possible values : `ABOVE`, `RANGE`. Listing both records both; listing one filters the rest out; omitting / empty records all supported types.
...
3. Classify each market — the interval shape comes from the `groupItemTitle` prefix (the reliable signal): `A-B` (or a `between` question) → `RANGE`; `<X` (or `less than`/`below`) → `RANGE` open-ended lower tail; `>X` (or `greater than`) → `RANGE` open-ended upper tail; a bare number `X` with an `above` question → `ABOVE` ladder strike. Then apply the `market_types` filter. `TOUCH` (reach/hit/dip), `Up`/`Down` directional, volatility-index, dominance and comparison markets are skipped. Note: `>X` / `greater than` is the **top of a range ladder** (`bitcoin-price-on-DATE` events), not a standalone `ABOVE` — so an event stays a consistent single type.
