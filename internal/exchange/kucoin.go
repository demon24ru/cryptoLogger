...
func (k *kucoin) subWsChannel(market string, channel string, id int) error {
	...
	case "ordersbook":
		channel = "/spotMarket/level2Depth50:" + market
	}
	sub := wsSubKucoin{
		ID:             id,
		Type:           "subscribe",
	...
	for {
		...
		default:
			...
			case "message":
				...
				if wr.Topic == "ticker" || wr.Topic == "trade" || wr.Topic == "level2" || wr.Topic == "ordersbook" {
...
func (k *kucoin) processWs(ctx context.Context, wr *respKucoin, cd *commitData) error {
	...
	case "ticker":
		...
		ticker.MktCommitName = strings.ReplaceAll(wr.mktCommitName, "-", "")
		ticker.Data = wr.data
	...
	case "trade":
		...
		trade.MktCommitName = strings.ReplaceAll(wr.mktCommitName, "-", "")
		trade.Data = wr.data
...
func (k *kucoin) processREST(ctx context.Context, mktCommitName string, channel string, interval int) error {
	...
	wrk := func() error {
		...
		case "ordersbook":
			...
			key := cfgLookupKey{market: mktCommitName, channel: "ordersbook"}
			val := k.cfgMap[key]
			if val.terStr {
				cd.terOrdersBookCount++
				cd.terOrdersBooks = append(cd.terOrdersBooks, ordersbook)
