...
func StartBinanceFutures(appCtx context.Context, markets []config.Market, retry *config.Retry, connCfg *config.Connection) error {
	...
	for {
		log.Error().Str("exchange", "binanceFutures").Msg(fmt.Sprintf("start %s", strings.Join(mks[:], ",")))
		...
		if err != nil {
			...
			log.Error().Str("exchange", "binanceFutures").Int("retry", retryCount).Msg(fmt.Sprintf("retrying functions in %d seconds %s", retry.GapSec, strings.Join(mks[:], ",")))
...
type wsRespLevel2BinanceFutures struct {
	Symbol string     `json:"s"`
	Bids   [][]string `json:"b"`
	Asks   [][]string `json:"a"`
}
	...
	for {
		...
		default:
			...
			if wr.Event == "ticker" || wr.Event == "trade" || wr.Event == "ordersbook" || wr.Event == "level2" {
				...
				case "level2":
					wrt := wsRespLevel2BinanceFutures{}
					err = jsoniter.Unmarshal(frame, &wrt)
					if err != nil {
						log.Debug().Str("exchange", "binanceFutures").Str("func", "readWs").Msg(string(frame))
						logErrStack(err)
