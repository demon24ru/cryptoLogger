...
func (c *ClickHouse) CommitTickers(appCtx context.Context, data []Ticker) error {
	...
	stmt, err := tx.Prepare("INSERT INTO ticker_? (data, timestamp) VALUES (?, ?)")
	if err != nil {
		return err
	}
	defer stmt.Close()
	...
	for i := range data {
		ticker := data[i]
		_, err := stmt.Exec(ticker.MktCommitName, ticker.Data, ticker.Timestamp.Format(clickHouseTimestamp))
		if err != nil {
			return err
		}
	}
...
func (c *ClickHouse) CommitTrades(appCtx context.Context, data []Trade) error {
	...
	stmt, err := tx.Prepare("INSERT INTO trade_? (data, timestamp) VALUES (?, ?)")
	if err != nil {
		return err
	}
	defer stmt.Close()
	...
	for i := range data {
		trade := data[i]
		_, err := stmt.Exec(trade.MktCommitName, trade.Data, trade.Timestamp.Format(clickHouseTimestamp))
		if err != nil {
			return err
		}
	}
...
func (c *ClickHouse) CommitLevel2(appCtx context.Context, data []Level2) error {
	...
	stmt, err := tx.Prepare("INSERT INTO level2_? (data, timestamp) VALUES (?, ?)")
	if err != nil {
		return err
	}
	defer stmt.Close()
	...
	for i := range data {
		level2 := data[i]
		_, err := stmt.Exec(level2.MktCommitName, level2.Data, level2.Timestamp.Format(clickHouseTimestampMicroSec))
		if err != nil {
			return err
		}
	}
