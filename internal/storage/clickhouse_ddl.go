...
// definitions below, copied verbatim) lives in scripts/clickhouse_schema.sql.
//
// This is a best-effort convenience so the pipeline works on a fresh database;
// it is called from the initializer when ClickHouse is enabled and is non-fatal
// on error (e.g. if the 'moving_from_ssd_to_hdd' storage policy is absent, the
// operator should apply scripts/clickhouse_schema.sql manually).

const chExchangeEnum = "Enum8('binance'=1,'kucoin'=2,'bybit'=3)"
...
func (c *ClickHouse) CreateExchangeTables(appCtx context.Context) error {
	...
	stmts := []string{
		...
		ddl("trade", "DateTime64(3, 'UTC')", "data String"),
		ddl("level2", "DateTime64(9, 'UTC')", "bids String,\nasks String"),
		ddl("ordersbook", "DateTime64(9, 'UTC')", "bids String,\nasks String"),
	}
